...
export const ModalForm: FC<IModalFilter> = (props: IModalFilter) => {
  ...
  const checkRating = (value: number) => {
    ...
    return Promise.reject(
      new Error(
        "Минимальное значение рейтинга, не может превышать максимального!"
      )
    );
  };
  ...
  return (
    ...
    >
      ...
      <Form.Item label="Максимальный рейтинг:" name="ratingTo">
        <InputNumber min={1} max={10} />
      </Form.Item>

      <Form.Item label="Временной промежуток:" name="years">
        <RangePicker picker="year" />
      </Form.Item>

      <Form.Item label="Жанр:" name="genre">
        <MovieSelect genres={filterList.genres} onChangeGenre={onChangeGenre} />
      </Form.Item>
